...
from backend.common.command.carpool_request_create_command import CarpoolRequestCreateCommand


class CarpoolRequestCreateCommandTestCase(TestCase):

    def test_properties(self):
        command = CarpoolRequestCreateCommand(from_location="A", to_location="B",
    ...
    def test_str(self):
        command = CarpoolRequestCreateCommand(from_location='A', to_location="B", rider_id=1, minimum_passenger="2")
        self.assertEqual(
            str(command),
