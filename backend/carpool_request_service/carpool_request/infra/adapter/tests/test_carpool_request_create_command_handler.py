...
class CarpoolRequestCreateCommandHandlerTestCase(TestCase):
    ...
    def test_when_message_has_no_email_or_password_raise_exception(self):
        carpool_request_create_command_handler = \
            CarpoolRequestCreateCommandHandler(carpool_request_application_service=None)

        with self.assertRaises(ValueError):
            carpool_request_create_command_handler.handle(
                CarpoolRequestCreateCommand(from_location=None, to_location=None, minimum_passenger=None, rider_id=None))
    ...
    def test_when_message_valid_field_then_call_register(self):
        ...
        class MockCarpoolRequestApplicationService:
            ...
            def create(self, from_location, to_location, minimum_passenger, rider_id):
                self.assert_func(from_location, to_location, minimum_passenger, rider_id)
        ...
        def assert_func(from_location, to_location, minimum_passenger, rider_id):
        ...
        carpool_request_create_command_handler.handle(CarpoolRequestCreateCommand(
            from_location=FROM_LOC, to_location=TO_LOC, minimum_passenger=MIN, rider_id=RIDER_ID))
